<div align="center">

<!-- HEADER BANNER -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=30%20Days%20of%20Code&fontSize=50&fontColor=fff&animation=twinkling&fontAlignY=35&desc=HackerRank%20Challenge%20Solutions&descSize=20&descAlignY=60" width="100%"/>

<!-- BADGES -->
<p>
  <img src="https://img.shields.io/badge/HackerRank-30%20Days%20of%20Code-00EA64?style=for-the-badge&logo=hackerrank&logoColor=white" alt="HackerRank"/>
  <img src="https://img.shields.io/badge/Language-C++20-3776AB?style=for-the-badge&logo=c++20&logoColor=white" alt="Python"/>
  <img src="https://img.shields.io/badge/Progress-14%2F30-FF6B35?style=for-the-badge" alt="Progress"/>
  <img src="https://img.shields.io/badge/Status-Active-brightgreen?style=for-the-badge" alt="Status"/>
</p>

</div>

---

## 🚀 About This Repository

> **HackerRank's 30 Days of Code** is a month-long challenge designed to help developers strengthen their programming foundations — covering data types, data structures, algorithms, and object-oriented design.

This repository contains my **clean, well-commented solutions** to every problem in the challenge. Each solution is written with clarity and best practices in mind — not just to pass the tests, but to be understood.

---

## 📋 Challenge Overview

| Detail | Info |
|--------|------|
| 🏆 Platform | [HackerRank – 30 Days of Code](https://www.hackerrank.com/domains/tutorials/30-days-of-code) |
| 💻 Primary Language | C++20 |
| 📅 Duration | 30 Days |
| 🎯 Difficulty | Beginner → Intermediate |
| 🔄 Updated | Regularly |

---

## 📂 Solutions Index

| Day | Problem | Concepts Covered | Solution |
|-----|---------|-----------------|----------|
| **Day 00** | Hello, World! | Printing, I/O | [View →](./Day%200%3A%20Hello%2C%20World.) |
| **Day 01** | Data Types | Integers, Doubles, Strings | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/5b1f57c5fa7669fa3b60da5ddcd35a30e395453a) |
| **Day 02** | Operators | Arithmetic, Modulo | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/dcf92f8d36922d3e0fd756b618fdd09307ccb850) |
| **Day 03** | Intro to Conditional Statements | if-else | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/dd96beed0b55122c487fb4f1e174706fc7281684) |
| **Day 04** | Class vs. Instance | OOP Basics | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/5ed659b6671a35088931ec58b3e284a0dead0a67) |
| **Day 05** | Loops | for, while | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/250130eafd7763dfc71b69c2ef78bce16af9fbb2) |
| **Day 06** | Let's Review | Strings | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/9318ca536f1c763b4af532d118aaf36b14abf704) |
| **Day 07** | Arrays | Lists | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/7bff0954bd85aed32aff01f9c5512887a6c95028) |
| **Day 08** | Dictionaries and Maps | Hash Maps | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/94a6b0547f76657a52202779df9cd12108a28af7) |
| **Day 09** | Recursion 3 | Factorial, Recursion | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/44baab0b098d09ab4170eacfe5d1c772c3500ea3) |
| **Day 10** | Binary Numbers | Bitwise | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/c01a46f5ecc80bc889bbc7aa7135615def9a7d92) |
| **Day 11** | 2D Arrays | Matrix Traversal | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/49783272c259b84c93bfe0cd1d390271799fe75f) |
| **Day 12** | Inheritance | OOP | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/26489bbeab201b3c6dad352d978d6359a65b1f52) |
| **Day 13** | Abstract Classes | OOP | [View →](https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code/commit/ab95dab526bf52c8a483ecba18f3e37a2189a78c) |
| **Day 14** | Scope | Variable Scope | 🔜 Coming Soon |
| **Day 15** | Linked List | Data Structures | 🔜 Coming Soon |
| **Day 16** | Exceptions — String to Integer | Error Handling | 🔜 Coming Soon |
| **Day 17** | More Exceptions | Try/Catch | 🔜 Coming Soon |
| **Day 18** | Queues and Stacks | Data Structures | 🔜 Coming Soon |
| **Day 19** | Interfaces | OOP | 🔜 Coming Soon |
| **Day 20** | Sorting | Bubble Sort | 🔜 Coming Soon |
| **Day 21** | Generics | Type System | 🔜 Coming Soon |
| **Day 22** | Binary Search Trees | Trees | 🔜 Coming Soon |
| **Day 23** | BST Level-Order Traversal | Trees, BFS | 🔜 Coming Soon |
| **Day 24** | More Linked Lists | Data Structures | 🔜 Coming Soon |
| **Day 25** | Running Time and Complexity | Big-O | 🔜 Coming Soon |
| **Day 26** | Nested Logic | Conditionals | 🔜 Coming Soon |
| **Day 27** | Testing | Unit Testing | 🔜 Coming Soon |
| **Day 28** | RegEx, Patterns, and Intro to Databases | Regex | 🔜 Coming Soon |
| **Day 29** | Bitwise AND | Bitwise Ops | 🔜 Coming Soon |

---

## 🧰 Topics Covered

<div align="center">

```
📦 Core Foundations
├── 🔢  Data Types & Variables
├── 🔀  Conditionals & Loops
├── 📐  Functions & Recursion
├── 🗂️  Arrays & Strings

🏗️  Data Structures
├── 🔗  Linked Lists
├── 📚  Stacks & Queues
├── 🌳  Binary Trees & BSTs
└── 🗺️  Dictionaries & Maps

🧬  Object-Oriented Programming
├── 🧱  Classes & Objects
├── 🧩  Inheritance
├── 🔭  Abstract Classes
└── 🔌  Interfaces

⚡  Advanced Concepts
├── 📊  Sorting Algorithms
├── 🔍  Binary Search
├── 🧮  Bitwise Operations
└── 🎯  Complexity Analysis
```

</div>

---

## ⚡ How to Use

```bash
# 1. Clone this repository
git clone https://github.com/dewanshikarnawat/Hackkerank_30_Days_of_Code.git

# 2. Navigate into the folder
cd Hackkerank_30_Days_of_Code

# 3. Open any solution
# Each solution file is organized by Day number

# 4. Run a solution (Python)
python "Day 0: Hello, World."
```

---

## 📈 Progress Tracker

```
Day  0  ████████████████████  ✅ Complete
Day  1  ████████████████████  ✅ Complete
Day  2  ████████████████████  ✅ Complete
Day  3  ████████████████████  ✅ Complete
Day  4  ████████████████████  ✅ Complete
Day  5  ████████████████████  ✅ Complete
Day  6  ████████████████████  ✅ Complete
Day  7  ████████████████████  ✅ Complete
Day  8  ████████████████████  ✅ Complete
Day  9  ████████████████████  ✅ Complete
Day  10  ████████████████████  ✅ Complete
Day  11  ████████████████████  ✅ Complete
Day  12  ████████████████████  ✅ Complete
Day  13  ████████████████████  ✅ Complete
...
Day 29  ░░░░░░░░░░░░░░░░░░░░  🔜 Upcoming

Overall  [██████████████░░░░░░]  14% (14/30 Days)
```

---

## 🙋‍♂️ About Me

<div align="center">

**Dewanshi Karnawat**
*Passionate Developer | Problem Solver | Lifelong Learner*

[![GitHub](https://img.shields.io/badge/GitHub-dewanshikarnawat-181717?style=for-the-badge&logo=github)](https://github.com/dewanshikarnawat)
[![HackerRank](https://img.shields.io/badge/HackerRank-Profile-00EA64?style=for-the-badge&logo=hackerrank&logoColor=white)](https://www.hackerrank.com)

</div>

---

## 🤝 Contributing

Found a more elegant solution? Spotted a bug? Contributions are welcome!

1. Fork the repo
2. Create your branch: `git checkout -b improve/day-XX`
3. Commit changes: `git commit -m 'Improve Day XX solution'`
4. Push: `git push origin improve/day-XX`
5. Open a Pull Request

---

## ⭐ Show Some Love

If this repository helped you in any way, please consider giving it a **⭐ star** — it motivates me to keep going and helps others discover it too!

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer" width="100%"/>

*"The best way to learn programming is to write programs." — Dennis Ritchie*

</div>
